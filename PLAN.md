...
- 卡片（单列，适配 ~300px，可 split/float）：日常（洗漱 2 格点、洗澡、洗衣：待洗数 + 攒 + **洗完(件数)→会话**+修正）、饮食（三餐，点即记时间，可加价格/清除）、学习（背单词：目标进度条 + 加会话 + 改当日目标；多邻国：列表 + 加一节）、健身（跑步表单：时长默认 30/里程/心率；跳绳 90/180 + 秒数 + 心率；引体支撑秒数；器材 名称/动作数/重量）、记录（媒体、任务迷你卡 + 添加入口）。
- 会话条目可点开编辑/删除（补卡同界面）。
...
- ③ 登记区：三餐（各餐天数与比例/Σ花费/按餐次花费）· 洗衣存量当前值 · 媒体（书影分开的完成数/平均评分/在列）· 任务（完成数/逾期完成数/当前待办·进行中·逾期计数）。
