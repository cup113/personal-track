...
| 6 | chip 分母 | **8 个习惯格硬编码**（洗漱 2 + 洗澡 1 + 三餐 3 + 背单词 1 + 多邻国 1 存在目标）；**当日到期任务各再占一格**，分母 = `8 + 当日到期任务数`（M9 推翻原"任务不计入分母"） |
...
| 派生值 | 规则 |
|---|---|
| 目标进度 | 背单词 = 加权分位（新词 ×5 + 复习 ×1，分子分母同权）；多邻国 = 存在性（≥1 节） |
| **格值** | 0..1 的分数：洗漱 `min(times.length,2)` 各一格 · 洗澡 `at≠null` · 三餐各有值 · **背单词 = 上限 1 的加权目标进度** · 多邻国 ≥1 节 · **每条当日到期任务一格 = `current/total`** → `n/N`，`N = 8 + 当日到期任务数`，计数保留一位小数 |
| 到期带 | `due − 今天` 的整天数：<0 逾期 · 0 当天 · ≤3 橙 · ≤7 黄 · 更远无强调（纯显示） |
...
- 顶部：**DateNav**（前后日 / 日历选择，禁未来；**夜段**显示"周X夜"）+ **完成条 `n/N`**（每格按自己的分数渐变填绿，悬停看百分比）。
...
| **M7 反馈一轮** | `9d9bd2a` | ✅ 八条反馈：去掉「每日重置」字样与分类筛选；打卡改为「记录 + 虚线空槽」；洗澡并入打卡卡；三餐改紧凑行；背单词复习词按 1/5 加权；统计页三餐只留早饭与总花费；**导出/导入**（事实文件 + 合并/覆盖两种导入）。统计面板滚动修复见 `c35867c` |
| **M8 表单初值** | `0de6d4d` | ✅ 会话（背单词/多邻国/健身/洗衣）**时间默认当前时刻**；新建任务**截止默认本周末**（周一为首，取当前习惯日所在周的周日）；客户端纯函数首次有独立的测试程序 |
| **M9 完成条变成分数** | 见提交 | ✅ **格**由布尔改为 0..1 分数（背单词填加权进度、当日到期任务各占一格填 `current/total`），计数形如 `5.3/9`，每格**渐变填绿**；任务按**截止升序**（未完成在前、无截止日在组末），截止日按逾期/当天/3 日/7 日**分档着色**；新增任务纯函数测试与 E2E「当日到期格」阶段 |

自动化验证入口：`pnpm typecheck`（宿主 / 浏览器 / 测试三个程序）、
`pnpm test`（40 项数据层断言 + 9 项客户端纯函数断言）、
`pnpm verify`（客户端 facade：包装契约/四个注册面/24 个命令 + 宿主 17 阶段真实栈端到端，
含"当日到期格 → 分数 → 删除回退"与"导出→空根导入→重算→合并/覆盖/坏文件拒绝"）。

### 尚未完成 / 需要在你的环境确认

| 项 | 说明 |
|---|---|
| **重启后的目视验收** | 宿主半身只在 `dsh` 重启时替换；M7 的导出/导入与三餐投影需要你重启一次 `dsh --profile web` 再看（M8 只改客户端，刷新页面即可） |
